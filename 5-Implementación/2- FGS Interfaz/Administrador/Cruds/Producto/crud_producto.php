	...
	<nav class="navbar navbar-expand-lg navbar-dark fixed-top" style="background-color:rgb(0, 0, 0);">
        ...
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
					...
					<div class="col-6">
					<form action="inser_producto.php">
						<button  class="btn btn-success" data-toggle="modal"><i class="material-icons">&#xE147;</i> <span>Agregar nuevo producto</span></button>						
					</form>
					</div>
                </div>
            </div>
		<?php
			...
			if(isset($_GET['aksi']) == 'delete'){
				...
				$cek = mysqli_query($con, "SELECT * FROM fgs_producto WHERE id_producto='$nik'");
				if(mysqli_num_rows($cek) == 0){
					echo '<div class="alert alert-info alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No se encontraron datos.</div>';
				}else{
					$delete = mysqli_query($con, "DELETE FROM fgs_producto WHERE id_producto='$nik'");
					if($delete){
            ...
            <table class="table table-striped table-hover">
				...
				$a = "SELECT * FROM fgs_producto
				JOIN fgs_proveedor ON
				...
				$sql = mysqli_query($con, $a);
					...
					while($row = mysqli_fetch_assoc($sql)){
						...
						<tr>
							...
							<td>.<img src="'.$row['imagen_producto'].'" width="100">.</td>
							<td>'.$row['fecha_vencimiento_producto'].'</td>
							<td>'.$row['precio'].'</td>
							<td>'.$row['marca'].'</td>
							<td>'.$row['primer_nombre_proveedor'].'</td>
							<td>';
							echo '
							<td>
								<a href="edit_producto.php?nik='.$row['id_producto'].'" title="Editar datos" class="btn btn-primary btn-sm"><i class="material-icons">edit</i></a>
								<a href="crud_producto.php?aksi=delete&nik='.$row['id_producto'].'" title="Eliminar" onclick="return confirm(\'¿Esta seguro de borrar la cita número '.$row['id_producto'].'?\')" class="btn btn-danger btn-sm"><i class="material-icons">delete</i></a>
							</td>
							</tr>
							';
					}
