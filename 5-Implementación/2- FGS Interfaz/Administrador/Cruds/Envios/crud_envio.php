	...
	<nav class="navbar navbar-expand-lg navbar-dark fixed-top" style="background-color:rgb(0, 0, 0);">
		...
		<?php
			...
			if(isset($_GET['aksi']) == 'delete'){
				...
				$cek = mysqli_query($con, "SELECT * FROM fgs_envio WHERE id_envio='$nik'");
				if(mysqli_num_rows($cek) == 0){
					echo '<div class="alert alert-info alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No se encontraron datos.</div>';
				}else{
					$delete = mysqli_query($con, "DELETE FROM fgs_envio WHERE id_envio='$nik'");
					if($delete){
				...
				$sql = mysqli_query($con, $a);
					...
					while($row = mysqli_fetch_assoc($sql)){
						...
						<tr>
							...
							echo '
							<td>
								<a href="edit_envio.php?nik='.$row['id_envio'].'" title="Editar datos" class="btn btn-primary btn-sm"><i class="material-icons">edit</i></a>
								<a href="crud_envio.php?aksi=delete&nik='.$row['id_envio'].'" title="Eliminar" onclick="return confirm(\'¿Esta seguro de borrar la cita número '.$row['id_envio'].'?\')" class="btn btn-danger btn-sm"><i class="material-icons">delete</i></a>
							</td>
							</tr>
							';
					}
