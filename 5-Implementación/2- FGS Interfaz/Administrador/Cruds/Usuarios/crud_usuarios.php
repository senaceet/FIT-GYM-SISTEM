...
<nav class="navbar navbar-expand-lg navbar-dark fixed-top" style="background-color:rgb(0, 0, 0);">
		...
		<?php
			...
			if(isset($_GET['aksi']) == 'delete'){
				...
				$cek = mysqli_query($con, "SELECT * FROM fgs_usuario WHERE numero_documento='$nik'");
				if(mysqli_num_rows($cek) == 0){
					echo '<div class="alert alert-info alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No se encontraron datos.</div>';
				}else{
					$delete = mysqli_query($con, "DELETE FROM fgs_usuario WHERE numero_documento='$nik'");
					if($delete){
            ...
            <table class="table table-striped table-hover">
                <thead>
                    <tr>
						...
						<th>Genero usuario</th>
						<th>Peso Usuario</th>
						<th>Estatura Usuario</th>
						<th>numero_telefono_usuario</th>
                    </tr>
                </thead>
                <tbody>
                <?php
                ...
                $a = "SELECT * FROM fgs_usuario WHERE id_rol>1";
                ...
                $sql = mysqli_query($con, $a);
					...
					while($row = mysqli_fetch_assoc($sql)){
						...
						<tr>
							...
							<td>'.$row['primer_nombre_usuario'].'&nbsp;'.$row['segundo_nombre_usuario'].'&nbsp;'.$row['primer_apellido_usuario'].'&nbsp;'.$row['segundo_apellido_usuario'].'</td>
							<td>'.$row['correo_usuario'].'</td>
							<td>'.$row['genero'].'</td>
							<td>'.$row['peso_usuario'].'</td>
							<td>'.$row['estatura_usuario'].'</td>
							<td>'.$row['numero_telefono_usuario'].'</td>
							<td>';
							echo '
							<td>
								<a href="crud_usuarios.php?aksi=delete&nik='.$row['numero_documento'].'" title="Eliminar" onclick="return confirm(\'¿Esta seguro de borrar la cita número '.$row['numero_documento'].'?\')" class="btn btn-danger btn-sm"><i class="material-icons">delete</i></a>
							</td>
							</tr>
							';
					}
